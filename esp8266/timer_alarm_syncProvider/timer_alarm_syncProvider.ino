...
void loadAlarms() {
  ...
  /* add alarm repeat update GMT time */
  Alarm.alarmRepeat(10, 40, 0, trigSyncProvider);
}
...
time_t syncTime() {
  ...
  /*
    ...
    if (!capnhat.sync("sangseu.github.io", "/gmt/index.html", 80)) {
      //setTime(capnhat.gio(), capnhat.phut(), capnhat.giay(), 1, 1, 11); // delault day-mont-year
      gmtNow.Hour = capnhat.gio();
      gmtNow.Minute = capnhat.phut();
      gmtNow.Second = capnhat.giay();
      tit();
      Serial.println("Synced time");
      i = 10; //set i>5 to break while
      break;
    }
    }
  */
  ...
  return makeTime(gmtNow);
}

void trigSyncProvider() {
  Serial.println("Call sync");
  now();
}
